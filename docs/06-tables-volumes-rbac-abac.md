...
> **📌 ABAC 기능 상태 (2026년 9월 기준)**
> - **행 필터(Row Filter)**: ✅ GA (정식 출시)
> - **열 마스킹(Column Mask)**: ✅ GA (정식 출시)
> - **카탈로그/스키마 레벨 ABAC 정책**: ✅ GA — 속성 기반 접근 제어(ABAC) 정책 프레임워크가 **2026년 4월 28일 정식 출시(GA)**되었습니다(정책의 조건으로 쓰는 **거버넌스 태그(governed tags)**는 2026년 4월 2일 GA).
> - **GRANT 정책(태그 기반 동적 권한 부여)**: ✅ GA
...
> ℹ️ **2026년 기준 참고**: 이 카탈로그/스키마 레벨 ABAC 정책 프레임워크는 **2026년 4월 정식 출시(GA)**되었으며, 실행에는 **서버리스 컴퓨트 또는 Databricks Runtime 16.4 이상**이 필요합니다(지원하지 않는 엔진에서는 정책이 '차단(fail-closed)'으로 동작해 데이터가 노출되지 않습니다). 반면 위 **7단계에서 실습한 SQL UDF 기반 행 필터·열 마스킹**은 DBR 12.2 LTS 이상에서 동작합니다.
