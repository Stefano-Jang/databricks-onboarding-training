⬅️ [이전: Lakeflow Connect로 S3 가져오기](./07b-lakeflow-connect-s3.md) | 🏠 [목차](../README.md) | [다음: Job & 스케줄](./09-jobs.md) ➡️
...
1. 왼쪽 사이드바에서 **SQL Editor** 아이콘(코드 편집기 모양)을 클릭합니다.
   ...
   ![SQL Editor 아이콘](../assets/screenshots/08-sql/01-sql-editor-icon.png)
   *📸 캡처 안내: 왼쪽 사이드바에서 SQL Editor 아이콘이 하이라이트된 상태를 캡처합니다.*

2. SQL 편집기 화면이 열립니다. 처음 열 경우 빈 쿼리 탭이 나타납니다.

   ![SQL 편집기 초기 화면](../assets/screenshots/08-sql/02-sql-editor-home.png)
...
1. 편집기 상단(또는 우측 상단)의 컴퓨트 선택 드롭다운을 클릭합니다. (화면에서 실제 명칭 확인)
   ...
   ![SQL Warehouse 선택 드롭다운](../assets/screenshots/08-sql/03-warehouse-dropdown.png)
...
4. 막대그래프가 표시됩니다.
   ...
   ![막대그래프 시각화](../assets/screenshots/08-sql/07-visualization.png)
...
1. 편집기 상단의 쿼리 제목 부분을 클릭합니다. 기본값은 **"New query"** 입니다.
   ...
   ![쿼리 제목 클릭](../assets/screenshots/08-sql/08-query-title.png)
...
3. 쿼리는 자동 저장됩니다. 저장 상태는 제목 옆의 아이콘(화면에서 실제 명칭 확인)으로 확인할 수 있습니다.
   ...
   ![저장된 쿼리 이름](../assets/screenshots/08-sql/09-saved-query-name.png)
...
2. 목록에서 `onboarding_nyc_taxi_by_zipcode` 를 클릭합니다.
   ...
   ![Queries 목록](../assets/screenshots/08-sql/10-queries-list.png)
...
3. 팀원 이메일 또는 그룹을 입력하고 권한 수준을 설정합니다.
   ...
   ![권한 설정 다이얼로그](../assets/screenshots/08-sql/11-share-permissions.png)
...
3. **Query snippets** 를 선택합니다.
   ...
   ![Query snippets 메뉴](../assets/screenshots/08-sql/12-snippets-menu.png)
   *📸 캡처 안내: SQL 편집기의 더보기(⋮) 메뉴가 열려 있고 "View" 하위에 "Query snippets" 항목이 보이는 화면을 캡처합니다.*

4. **Query Snippets** 관리 페이지가 열립니다.

   ![Query Snippets 관리 화면](../assets/screenshots/08-sql/13-snippets-page.png)
...
2. 아래와 같이 입력합니다.
   ...
   **Snippet** 필드에 입력할 SQL:
   ...
   ![스니펫 생성 폼](../assets/screenshots/08-sql/14-create-snippet-form.png)
...
2. 편집 영역에서 트리거 단어 **`nyc_top_zip`** 을 입력하기 시작합니다.
   ...
   ![자동완성에 스니펫 표시](../assets/screenshots/08-sql/15-snippet-autocomplete.png)
...
4. 스니펫의 전체 SQL 코드가 편집기에 삽입됩니다.
   ...
   ![스니펫 삽입 결과](../assets/screenshots/08-sql/16-snippet-inserted.png)
...
6. **Run** 버튼 또는 **Ctrl + Enter**로 실행합니다.
   ...
   ![스니펫 쿼리 실행 결과](../assets/screenshots/08-sql/17-snippet-result.png)
...
- [09. Job & 스케줄](./09-jobs.md) — Lakeflow Job을 만들고 스케줄을 설정해 쿼리와 노트북을 자동으로 실행하는 방법을 배웁니다.
