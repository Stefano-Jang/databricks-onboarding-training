...
1. 왼쪽 사이드바에서 **SQL Editor** 아이콘(코드 편집기 모양)을 클릭합니다.
   ...
   ![SQL Editor 아이콘](../assets/screenshots/09-sql/01-sql-editor-icon.png)
...
2. **Run** 버튼을 클릭하거나 **Ctrl + Enter** (Mac: **Cmd + Enter**)를 눌러 쿼리를 실행합니다.
   ...
   ![Run 버튼](../assets/screenshots/09-sql/05-run-button.png)
...
1. 편집기 상단의 쿼리 제목 부분을 클릭합니다. 기본값은 **"New query"** 입니다.
   ...
   ![쿼리 제목 클릭](../assets/screenshots/09-sql/08-query-title.png)
...
3. 쿼리는 자동 저장됩니다. 저장 상태는 제목 옆의 아이콘(화면에서 실제 명칭 확인)으로 확인할 수 있습니다.
   ...
   ![저장된 쿼리 이름](../assets/screenshots/09-sql/09-saved-query-name.png)
...
2. 목록에서 `onboarding_nyc_taxi_by_zipcode` 를 클릭합니다.
   ...
   ![Queries 목록](../assets/screenshots/09-sql/10-queries-list.png)
...
3. 팀원 이메일 또는 그룹을 입력하고 권한 수준을 설정합니다.
   ...
   ![권한 설정 다이얼로그](../assets/screenshots/09-sql/11-share-permissions.png)
...
3. **Query snippets** 를 선택합니다.
   ...
   ![Query snippets 메뉴](../assets/screenshots/09-sql/12-snippets-menu.png)
   *📸 캡처 안내: SQL 편집기의 더보기(⋮) 메뉴가 열려 있고 "View" 하위에 "Query snippets" 항목이 보이는 화면을 캡처합니다.*

4. **Query Snippets** 관리 페이지가 열립니다.

   ![Query Snippets 관리 화면](../assets/screenshots/09-sql/13-snippets-page.png)
...
2. 아래와 같이 입력합니다.
   ...
   **Snippet** 필드에 입력할 SQL:
   ...
   ![스니펫 생성 폼](../assets/screenshots/09-sql/14-create-snippet-form.png)
...
2. 편집 영역에서 트리거 단어 **`nyc_top_zip`** 을 입력하기 시작합니다.
   ...
   ![자동완성에 스니펫 표시](../assets/screenshots/09-sql/15-snippet-autocomplete.png)
...
4. 스니펫의 전체 SQL 코드가 편집기에 삽입됩니다.
   ...
   ![스니펫 삽입 결과](../assets/screenshots/09-sql/16-snippet-inserted.png)
...
6. **Run** 버튼 또는 **Ctrl + Enter**로 실행합니다.
   ...
   ![스니펫 쿼리 실행 결과](../assets/screenshots/09-sql/17-snippet-result.png)
