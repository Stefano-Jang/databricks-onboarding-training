...
1. 왼쪽 사이드바에서 **대시보드(Dashboards)** 아이콘을 클릭합니다.
   ...
   <!-- 스크린샷 예정: ../assets/screenshots/11-dashboards/01-dashboards-sidebar.png (Dashboards 아이콘) -->
...
3. 새 대시보드 편집 화면이 열립니다. 화면 상단의 대시보드 이름 필드를 클릭하고 이름을 입력합니다.
   ...
   <!-- 스크린샷 예정: ../assets/screenshots/11-dashboards/03-dashboard-name.png (대시보드 이름 입력) -->
...
1. 화면 왼쪽 상단에서 **데이터(Data)** 탭을 클릭합니다.
   ...
   <!-- 스크린샷 예정: ../assets/screenshots/11-dashboards/04-data-tab.png (Data 탭) -->
   *📸 캡처 안내: "Data" 탭이 선택된 대시보드 편집 화면.*

2. **SQL 데이터셋 추가(Add SQL dataset)** 또는 **데이터 추가(Add data)** 버튼을 클릭합니다.

   <!-- 스크린샷 예정: ../assets/screenshots/11-dashboards/05-add-dataset.png (데이터셋 추가 버튼) -->
...
3. SQL 편집기가 열립니다. 데이터셋 이름을 `pickup_zip_stats` 로 변경한 뒤, 아래 SQL을 입력합니다.
   ...
   <!-- 스크린샷 예정: ../assets/screenshots/11-dashboards/06-dataset-sql.png (SQL 데이터셋 쿼리 입력) -->
   *📸 캡처 안내: SQL 편집기에 위 쿼리가 입력된 상태. 데이터셋 이름 "pickup_zip_stats"가 표시되도록.*

4. **실행(Run)** 버튼(▷ 아이콘)을 클릭해 쿼리 결과를 확인합니다. 결과 테이블이 아래에 표시되면 정상입니다.

   <!-- 스크린샷 예정: ../assets/screenshots/11-dashboards/07-dataset-result.png (쿼리 결과 확인) -->
...
6. 같은 방법으로 두 번째 데이터셋을 추가합니다. **데이터 추가(Add data)** 를 다시 클릭하고 이름을 `daily_trips` 로 설정한 뒤 아래 SQL을 입력합니다.
   ...
   <!-- 스크린샷 예정: ../assets/screenshots/11-dashboards/08-dataset-daily.png (두 번째 데이터셋) -->
...
3. **시각화(Visualization)** 를 선택합니다.
   ...
   <!-- 스크린샷 예정: ../assets/screenshots/11-dashboards/10-add-widget.png (위젯 추가 메뉴) -->
...
4. 위젯 설정 패널이 오른쪽에 열립니다. 다음과 같이 설정합니다.
   ...
   <!-- 스크린샷 예정: ../assets/screenshots/11-dashboards/11-bar-chart-config.png (막대 차트 설정) -->
...
6. 캔버스에 막대 차트가 표시됩니다.
   ...
   <!-- 스크린샷 예정: ../assets/screenshots/11-dashboards/12-bar-chart-result.png (막대 차트 결과) -->
...
3. 제목을 `일별 운행 건수 추이` 로 입력합니다.
   ...
   <!-- 스크린샷 예정: ../assets/screenshots/11-dashboards/13-line-chart-config.png (선 차트 설정) -->
   *📸 캡처 안내: 선 차트 위젯 설정 패널. Line 유형이 선택된 상태.*

   <!-- 스크린샷 예정: ../assets/screenshots/11-dashboards/14-line-chart-result.png (선 차트 결과) -->
...
6. 제목을 `총 운행 건수` 로 입력합니다.
   ...
   <!-- 스크린샷 예정: ../assets/screenshots/11-dashboards/15-counter-config.png (카운터 설정) -->
   *📸 캡처 안내: Counter 유형이 선택된 위젯 설정 패널. Value = daily_trip_count, Aggregation = Sum.*

   <!-- 스크린샷 예정: ../assets/screenshots/11-dashboards/16-counter-result.png (카운터 결과) -->
...
1. 캔버스 빈 공간에서 **추가(+)** 를 클릭하고 **필터(Filter)** 를 선택합니다.
   ...
   <!-- 스크린샷 예정: ../assets/screenshots/11-dashboards/17-add-filter.png (필터 추가 메뉴) -->
...
3. 다음과 같이 설정합니다.
   ...
   <!-- 스크린샷 예정: ../assets/screenshots/11-dashboards/18-date-filter-config.png (날짜 필터 설정) -->
...
4. **필터 저장** 후, 선 차트 위젯을 클릭하고 설정 패널에서 **필터(Filters)** 섹션을 찾아 방금 만든 날짜 필터를 연결합니다.
   ...
   <!-- 스크린샷 예정: ../assets/screenshots/11-dashboards/19-filter-link.png (필터 연결) -->
   *📸 캡처 안내: 선 차트 위젯 설정 패널에서 날짜 필터가 연결된 상태.*

5. 캔버스에서 날짜 범위 필터를 조작해 보면 연결된 차트가 함께 변경되는 것을 확인할 수 있습니다.

   <!-- 스크린샷 예정: ../assets/screenshots/11-dashboards/20-filter-action.png (필터 동작 확인) -->
...
1. 화면 오른쪽 상단의 **게시(Publish)** 버튼을 클릭합니다.
   ...
   <!-- 스크린샷 예정: ../assets/screenshots/11-dashboards/21-publish-button.png (Publish 버튼) -->
...
2. 게시 옵션 대화 상자가 열립니다. **데이터 권한(Data credentials)** 설정이 표시됩니다.
   ...
   <!-- 스크린샷 예정: ../assets/screenshots/11-dashboards/22-publish-options.png (게시 옵션) -->
...
1. 화면 오른쪽 상단의 **공유(Share)** 버튼 또는 **...** 메뉴에서 **공유(Share)** 를 선택합니다.
   ...
   <!-- 스크린샷 예정: ../assets/screenshots/11-dashboards/24-share-button.png (Share 버튼) -->
...
2. 공유 대화 상자가 열립니다. 사용자 또는 그룹을 검색해 추가하고 권한 수준을 지정합니다.
   ...
   <!-- 스크린샷 예정: ../assets/screenshots/11-dashboards/25-share-dialog.png (공유 대화 상자) -->
...
5. **완료(Done)** 를 클릭해 공유 설정을 저장합니다.
   ...
   <!-- 스크린샷 예정: ../assets/screenshots/11-dashboards/26-share-done.png (공유 완료) -->
